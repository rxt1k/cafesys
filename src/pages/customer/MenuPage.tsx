...
export default function MenuPage() {
  ...
  async function fetchMenuData() {
    ...
    try {
      ...
      if (catRes.error) {
        throw new Error(catRes.error.message || "Failed to fetch categories");
      }
      if (dishRes.error) {
        throw new Error(dishRes.error.message || "Failed to fetch dishes");
      }
  ...
  const handleResetSession = () => {
    ...
    localStorage.removeItem(STORAGE_KEYS.TABLE_INFO);
    ...
    <div className="min-h-screen bg-app">
      ...
      <div className="sticky top-0 z-30 bg-app border-b border-default">
        <div className="px-4 pt-4 pb-3">
          ...
          {!sessionHealthy && (
            <div className="mb-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 dark:border-red-900/40 dark:bg-red-900/20">
              <div className="flex items-start gap-2">
                <ShieldAlert className="w-4 h-4 text-red-600 mt-0.5" />
                <div className="flex-1">
                  <p className="text-xs text-red-700 dark:text-red-300 font-medium">
                    Session seems inactive. You may not be able to place orders.
                  </p>
                  <button
                    onClick={handleResetSession}
                    className="mt-1 text-xs text-red-700 dark:text-red-300 underline"
                  >
                    Rescan QR and start new session
                  </button>
                </div>
              </div>
            </div>
        ...
        {error ? (
          ...
          <>
            ...
            {activeCategory === "all" && !searchQuery && (
              <div className="mb-6">
                <PopularSection
                  dishes={dishes.filter((d) => d.is_popular)}
            ...
            {activeCategory === "all" && !searchQuery ? (
              categories.map((cat) => {
                const catDishes = filtered.filter(
        ...
        {dishes.slice(0, 6).map((dish, i) => (
          ...
          >
            <button onClick={() => onSelect(dish)} className="w-full text-left">
              ...
              <p className="text-xs font-medium text-primary line-clamp-2 leading-snug">
                {dish.name}
              </p>
